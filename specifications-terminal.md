...
**Retenu (25 septembre 2026).** Le panneau de droite porte deux onglets, « Fichiers » et « Git » ; le bouton de la barre d’onglets l’ouvre ou le ferme sur sa dernière vue, mémorisée par onglet avec son ouverture. Commande « Afficher / masquer Git » dans la palette, Leader puis G ou Ctrl + Maj + G : ouvre la vue Git avec le focus, y bascule depuis les fichiers, ou ferme et rend le focus au terminal. La vue suit le dépôt ou le worktree du dossier du pane actif et affiche « Aucun dépôt Git » hors dépôt ; elle se met à jour en direct, y compris après une commande tapée au terminal ou lancée par un agent. En-tête : dépôt, branche ou HEAD détachée, avance et retard sur la branche distante suivie, boutons Récupérer, Tirer, Pousser (qui publie une branche sans amont), Annuler et Actualiser. Trois onglets internes : Modifications (conflits, fichiers indexés et non indexés, actions par fichier ou pour tout le groupe, message de commit, « Modifier le dernier commit », « Commit » et « Commit et push »), Historique (graphe des branches locales, des branches distantes et des tags, bascule « Courante » limitée à la branche courante et à son amont, pages de 200 commits chargées au défilement ; menu d’un commit : voir, extraire, créer une branche ou un tag, cherry-pick, fusionner, rebaser, reset soft, mixed ou hard) et Branches (branches locales avec avance et retard, branches distantes par dépôt distant, tags et stash ; double-clic pour basculer, menus pour fusionner, rebaser, renommer, supprimer, pousser un tag, appliquer ou supprimer un stash). Un diff s’affiche dans un volet large accolé au panneau, par-dessus les terminaux : il suit le fichier choisi, se recharge quand le fichier change et se ferme par Échap ou la croix ; pour un commit, il montre le message, les fichiers et le diff du fichier choisi. Les branches distantes se suppriment depuis la vue après confirmation ; le reset hard, la suppression d’une branche non fusionnée, l’abandon de modifications, la suppression d’un stash, l’abandon d’une opération en cours et le push forcé demandent aussi une confirmation. « Annuler » couvre commit, modification du dernier commit, fusion, pull, rebase, cherry-pick, reset (les modifications locales effacées par un reset hard sont restaurées), bascule, création, suppression et renommage de branche, tags, remise en stash, suppression d’un stash et abandon de modifications ; il est refusé, raison en infobulle, si le dépôt a changé depuis ou si les commits ont été poussés, ne couvre ni ce qui a été publié ni l’application d’un stash, et n’est pas conservé à la fermeture de Dock. Un conflit bascule sur Modifications avec une bannière « Terminer » / « Abandonner » ; « Marquer résolu » demande confirmation si le fichier contient encore des marqueurs de conflit. Après un push refusé, une bannière propose « Tirer » ou « Forcer le push… » avec `--force-with-lease`, refusé si la branche distante a changé depuis la dernière récupération. L’hôte exécute le `git` installé avec des arguments séparés, sans composer de ligne de commande, et respecte la configuration, les hooks et le gestionnaire d’identifiants de l’utilisateur ; la sortie d’une commande refusée, hooks compris, s’affiche dans la vue. La gestion des worktrees reste celle de `wtr`/`rmwt` ; les pull requests, le rebase interactif, l’indexation d’une partie de fichier et l’éditeur de conflits intégré sont hors périmètre.
