...
**Conventions proposées.** La vue cible le dépôt ou le worktree du dossier du pane actif et le suit comme l’explorateur ; hors dépôt, elle affiche « Aucun dépôt Git ». Elle se met à jour quand le dépôt change, y compris depuis le terminal ou un agent. Commande « Afficher / masquer Git » dans la palette, Leader puis G ou Ctrl + Maj + G. L’hôte exécute le `git` installé avec des arguments séparés, sans composer de ligne de commande, et respecte la configuration, les hooks et le gestionnaire d’identifiants de l’utilisateur. Le reset hard, la suppression d’une branche non fusionnée et l’abandon de modifications demandent une confirmation. Le graphe charge l’historique par pages. La gestion des worktrees reste celle de `wtr`/`rmwt` ; les pull requests, le rebase interactif et l’éditeur de conflits intégré sont hors périmètre.
