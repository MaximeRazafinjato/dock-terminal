...
Web → hôte : `app.ready`, `session.save {session}`, `terminal.create {pane, shell, cwd, cols, rows}`, `terminal.input {pane, data}`, `terminal.resize {pane, cols, rows}`, `terminal.ack {pane, chars}`, `terminal.close {pane}`, `projects.list`, `window.close`.

Hôte → web : `app.hello {session, shells, home}`, `terminal.created {pane, pid}`, `terminal.output {pane, data}`, `terminal.cwd {pane, path}`, `terminal.exit {pane, code}`, `projects.listed {root, projects[{name, path}], error?}`, `error {pane?, message}`.
