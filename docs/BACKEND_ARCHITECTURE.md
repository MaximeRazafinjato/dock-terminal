...
| `Shell/` | `ShellCatalog` (profils PowerShell 5.1 par défaut, PowerShell 7, CMD, Git Bash, disponibilité par présence du fichier), `PowerShellIntegration` (wrapper de prompt encodé en base64 passé par `-EncodedCommand`, émet `OSC 7`), `OscCwdParser` (automate qui extrait le dossier courant du flux de sortie). |
| `Session/` | Modèles `SessionModel`, `WorkspaceModel`, `TabModel`, `SplitNodeModel`, `PaneModel` ; `SessionValidator` (limites de profondeur, ratios, identifiants actifs) ; `SessionRepository` (lecture / écriture atomique de `%LOCALAPPDATA%\Dock\session.json`, refus d'une session invalide) ; `SessionFactory` (session initiale, identifiants). |
