...
| `Terminal/` | `PseudoConsole` (pipes + handle ConPTY), `PtyProcess` (`CreateProcessW` avec l'attribut pseudo-console, `STARTF_USESTDHANDLES` à vide), `JobObject` (kill-on-close), `TerminalSession` (assemble les trois, thread de lecture, événements `OutputReceived`, `CurrentDirectoryChanged`, `Exited` ; `ActiveProcessNames` liste les noms des processus encore vivants dans le Job Object hors le shell lui-même), `TerminalManager` (une session par identifiant de pane ; `Activity` renvoie, pour les panes demandés ou pour tous, ceux qui ont encore des programmes actifs sous la forme `PaneActivityModel(PaneId, Processes)`). |
...
| `Bridge/PathPicker.cs` | Sélecteurs natifs initialisés avec le handle de la fenêtre : fichier exécutable (`.exe`, `.cmd`, `.bat`, tout) ou dossier pour les champs de chemin, fichier `.json` à ouvrir et fichier `.json` à enregistrer (`FileSavePicker`) pour l'import / export des préférences. Annulation = aucun message. |

## Contrat du pont

Web → hôte : `app.ready`, `session.save {session}`, `text.save {text: {paneId: texte}}`, `settings.get`, `settings.save {settings}`, `settings.export`, `settings.import`, `dialog.pick {field, target: file|folder}`, `terminal.create {pane, shell, cwd, cols, rows}`, `terminal.input {pane, data}`, `terminal.resize {pane, cols, rows}`, `terminal.ack {pane, chars}`, `terminal.close {pane}`, `terminal.activity {panes[]}`, `projects.list`, `context.query {pane, path}`, `context.open {pane, path, target: editor|explorer}`, `window.close`, `window.closeCancel`.

Hôte → web : `app.hello {session, shells, home, text, persistence{textIntervalSeconds, linesPerPane}, recovery?}`, `app.closing {activity[{paneId, processes[]}]}`, `session.saved`, `session.saveFailed {message}`, `dialog.picked {field, path}`, `settings.result {settings, shellSettings[{id, name, defaultExecutable, configured, available}], files, warnings[], shells, persistence, saved}`, `settings.exported {path}`, `settings.imported {settings, path}`, `terminal.created {pane, pid}`, `terminal.output {pane, data}`, `terminal.cwd {pane, path}`, `terminal.exit {pane, code}`, `terminal.pathMissing {pane, path, fallback}`, `terminal.activityResult {panes[{paneId, processes[]}]}`, `projects.listed {root, projects[{name, path}], error?}`, `context.result {pane, path, git{isRepository, branch, detachedHead}}`, `error {pane?, message}`.
