...
| `src/terminal/` | `terminalRegistry` (une instance xterm.js par pane, conservée hors React pour survivre aux changements d'onglet ; renderer WebGL, repli canvas puis DOM ; acquittements de flux ; `snapshot` sérialise le texte d'un pane via `@xterm/addon-serialize`, `prime` réinjecte ce texte suivi d'un séparateur avant de lancer un nouveau processus), `tabLifecycle` (fermeture d'un onglet ou d'un pane avec capture du texte, réouverture du dernier onglet fermé), `TerminalPane` (composant qui attache l'instance à son élément). |
