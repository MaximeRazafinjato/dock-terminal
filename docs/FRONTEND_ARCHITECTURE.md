...
| `src/components/` | Un composant par fichier : `AppShell`, `Header`, `WorkspaceTree`, `TabBar`, `SplitView` (récursif), `PaneView`, `InlineNameEditor` (renommage inline : Entrée ou perte de focus valide, Échap annule ; le store ignore un nom vide), `SidebarResizer` (séparateur `role="separator"` : glisser à la souris ou flèches gauche/droite au clavier, largeur bornée par le store), `ShellMenu` (menu `role="menu"` ouvert par clic droit, Maj + F10 ou touche Menu sur le « + » de la barre d’onglets ; liste les shells disponibles annoncés par l’hôte, flèches / Début / Fin, Entrée, Échap, Tab ou clic extérieur). |
